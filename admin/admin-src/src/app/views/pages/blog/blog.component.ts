...
export class BlogComponent implements OnInit {
	...
	chngmode:Boolean; changeUploadTriggered:any;
	token = localStorage.getItem('Key'); per = false; access:any;
	config:any;

		model = {"_id":"","title":"","content":"","news_image":"","status":1};
  	...
  	constructor(
		...
		this.dataService.getRequest('admin/admin_access',this.token).subscribe((res:any) => {
			if(res.status == true){
				...
				var itemArr = [];
				itemArr = this.access.filter( item => item == 'Manage Asset Info');
				if(itemArr.length!=0){
					this.per = true;
				}else{
	...
	public getAll = () => {
		...
		this.dataService.getData('cms/get_blog')
		.subscribe(res => {
	...
	public redirectToAdd = () => {
		...
		this.pageType = 'add';
		this.newsUrl = "";
		this.model = {"_id":"","title": "", "content":"", "news_image":"", "status":1};
	}
	...
	public redirectToEdit = (data) => {
		...
		this.chngmode=true;
		this.pageType = 'edit';
		this.model = {"_id":data._id,"title":data.title,"content":data.content,"status":sts,"news_image":""};
	...
	doDelete (id: string, confirmed) {
		...
		this.dataService.getData('cms/blogDelete/'+id)
		.subscribe(reqData => {
			if(reqData.status == true) {
				this.getAll();
				this.notifier.notify( 'success', reqData.msg);
